...
# -*- coding: utf-8 -*-
"""router 层测试：create_new_project 的错误分类契约（I1）。

验证：
  - 无冲突 writing_config → 201 创建成功；
  - 硬冲突 → 400，detail 即服务层给出的冲突文案（明确）；
  - 其它非冲突 ValueError → 不被误标为 400（正常 500）。
"""
...
def test_create_project_no_conflict_returns_201(client):
    res = client.post(
        "/api/projects",
        json={"name": "无冲突项目", "writing_config": {"core_genre": "玄幻", "background": "宗门林立"}},
    )
...
def test_create_project_other_value_error_not_mislabeled_400(client):
    # 非冲突 ValueError（如内部数据错误）不应被当作「写作配置冲突」以 400 返回，而应正常 500
    with patch("app.routers.projects.create_project", side_effect=ValueError("内部数据错误")):
        res = client.post(
            "/api/projects",
            json={"name": "内部错误项目", "writing_config": {"core_genre": "玄幻"}},
        )
