...
- **原因**：一次续写 k 章时，正文串行 LLM 长跑（40 章 ≈ 40-60 分钟）绑架任务状态，目录几秒生成完却要等正文全部结束 UI 才刷新；用户无法在正文生成前审阅新目录
- **正文生成**：用户确认目录后，在章节页「AI 批量生成」（`batch_chapters`，增量语义：跳过已有 draft 章节）触发，只生成新章节正文
- **前端**：续写弹窗文案改为"本次续写将追加第 N+1~N+k 章目录（不生成正文），确认目录后可到章节页「AI 批量生成」生成正文"；`getTaskStepLabel` 新增 `continue_writing` 阶段文案（初始化/生成续写目录/保存续写目录）
- 任务校验（open 形态、k ≥ 1、N+k ≤ M）与 F1 目录累积落库逻辑不变
