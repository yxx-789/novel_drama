...
## D016：生成管线 LLM 模型选择 —— 规避推理模型

| 字段 | 内容 |
|------|------|
| **背景** | 接入 DeepSeek 后，生成任务长时间无输出（架构 5 分钟卡死）。排查发现 `deepseek-v4-flash` 是重度推理模型：在 `max_tokens=2048` 预算下把全部 token 消耗在 `reasoning_content`（思考过程），`content` 返回空，应用对空响应重试最多 3 次，5 步串行的架构管线累计 5 分钟以上无结果 |
| **选项** | A. `deepseek-v4-flash`（推理模型，2048 预算下 `content`=0 字、reasoning=7111 字）；B. `deepseek-v4-pro`（能正常输出，但 8192 预算下单次调用 >2 分钟，管线太慢）；C. `deepseek-chat`（非推理，实测 39s 完整输出 5366 字、`finish=stop`） |
| **决策** | **`deepseek-chat` + `LLM_MAX_TOKENS=8192`** |
| **理由** | 1. 非推理模型直接输出正文，无空响应重试问题；2. 8192 预算保证章节草稿（约 3000 token）不被截断；3. 39s/5366 字的实测速度可接受（架构 5 步约 1.5-2 分钟） |
| **影响** | 生成管线全链路（架构/目录/章节/短剧）使用 `deepseek-chat`；前端设置抽屉占位、`config.py` 默认值、`docs/API_SPEC.md` 示例同步更新 |
| **可逆性** | 高。纯环境配置（`.env`），改回任意模型即可；但注意：**换回推理模型（v4-flash 类）会复现空输出卡死**，除非同步调大 max_tokens 或修改 adapter 处理 reasoning_content |

---

## D017：生成系统去危机化 —— structure 条件化 + 题材伏笔/节奏方法论

| 字段 | 内容 |
|------|------|
| **背景** | 体检报告 A/F 缺陷：生成系统把「危机形状」当作普适硬规则（核心种子必须"否则灾难后果"、首章必须"埋异常征兆 + 打破平衡"、目录必须"悬念曲线"），导致日常流/种田/群像类项目也被强行危机化、AI 味重；同时题材伏笔/节奏方法论只有一句"伏笔手法倾向"，不够可执行。用户首要目标为整体提质量 + 兼顾速度，LLM 评审只设计不执行 |
| **选项** | A. 全局删除危机硬规则（会改变升级打怪/悬疑等危机驱动题材的既有质量，且违背"不删除功能"原则）；B. 按 structure 条件化：危机结构沿用现状基线，平静结构替换为平静/正反馈分片（保留人物魅力+生活切片+正反馈）；C. 只改文案淡化（治标不治本，且危机结构也受影响） |
| **决策** | **B：`build_structure_guidance(structure)` 返回 6 分片（seed/character/world/first_chapter/chapter/blueprint），危机结构（升级打怪/三幕经典/倒叙钩子/单元剧快节奏/长线连载）沿用现状文本为默认基线，平静结构（日常流/群像交织）替换为平静/正反馈分片；另新增 `genre_methodology.py` 题材参数表（伏笔间距/爽点频率/冲突驱动/钩子偏好），首章与续章注入【题材写作方法】片段，章末钩子形式化为四断法（决定/发现/误判/代价）+ 题材偏好** |
| **理由** | 1. 旧项目/缺省 structure 回退危机基线，行为与改造前完全一致，不破坏既有功能；2. 危机驱动题材质量不受影响；3. 只改 prompt 拼装，单章生成 **不新增 LLM 调用**（P3-A 前后均为 14 处 `_invoke_with_retry`）；4. 题材参数独立模块可测可扩展（对齐既有决策 D2） |
| **影响** | `prompts.py` 7 处硬危机规则改条件占位符；`generation_service` 组装时注入结构分片 + 题材方法论 + 钩子偏好；`_structure_for_project` 从 `writing_config.structure` 读结构（旧项目 None → 危机基线）；记忆轻量中性化（平静结构下 `extract_world_state_delta` / `build_state_summary` 追加"保留常态状态/舒适日常基调"约束、slim_state 平静放宽为 20 条/5 章窗口，缺省不变）；文档 CHANGELOG「V3 P3-A」、roadmap 阶段 2 →「已实现」 |
| **可逆性** | 高。全部改动为 prompt 条件化 + 参数透传，无数据迁移；回退仅需恢复 prompts.py 原文 + 移除注入调用 |
...
## D019：P3-B 闭环 —— L3 纵览 + known_by 信息约束 + 收尾修复

| 字段 | 内容 |
|------|------|
| **背景** | D018 交付后复核发现「三处写而不读」：① L3 全书摘要只写不读（`_build_l2_foreshadowing_context` 从不读取 `book_summary`）；② `known_by`（谁知晓该秘密）无下游消费者，防「角色说出不该知道的事」的 OOC 拦截形同虚设；③ 单章路径从不合成 L3（只有批量路径循环结束合成），逐章生成整本书时 L3 永远不存在。另有收尾项：`ARC_SIZE` 硬编码（spec 承诺「可配」未兑现）、台账无条件写回（无变化也 bump version）、单章路径零测试覆盖 |
| **选项** | A. L3 常驻注入（每章都塞全书摘要，token 成本高、稀释近端注意力）；B. **仅当伏笔提醒命中时注入 L3**（需回溯早期细节时才给全书脉络）；C. known_by 全量注入（token 膨胀）；D. **每章注入最近触碰前 5 条 + 提醒命中伏笔**（并集去重，覆盖最相关） |
| **决策** | **B + D**：L3 仅伏笔提醒命中时注入【全书脉络】；known_by 每章注入最近触碰 5 条 ∪ 提醒命中项渲染【信息约束】；单章路径在全书最后一章（`chapter_num == num_chapters`）合成一次 L3（摊薄 1/N）；`ARC_SIZE` 配置化（env `ARC_SIZE`，默认 15，模块加载时读一次保留模块级名，既有 mock 测试零改动）；台账写回加 deepcopy 快照 + existed 守卫，无变化跳过；**并发竞态本期不修**（arc 摘要冻结并发重复一次 LLM、数据幂等、任务串行实际不触发，DB 行锁改动大） |
| **理由** | 1. L3 仅在提醒命中时注入，既补上「纵览防幻觉」又避免常驻 token；2. known_by 拦截 OOC 是 D018 设计初衷的闭环兑现（不用上就等于白设计）；3. 单章合成 L3 与批量行为对称，逐章整本生成也能拿到全书脉络；4. 全部**零新增每章 LLM 调用**（L2/L3 摊薄 1/N，book 不计入每章预算）；5. 旧项目兼容（num_chapters 未设跳过、台账缺失仍初始化）；6. 收尾修复不删代码、不削弱既有功能 |
| **影响** | `foreshadowing_ledger.py` 新增纯规则 `build_known_by_constraints` + `_reminder_flags` 抽取（reminder 输出逐字节不变）；`task_service.py` `_build_l2_foreshadowing_context` 追加【全书脉络】+【信息约束】、`_merge_foreshadowing_ledger` 无变化跳过写回、`run_chapter_task` 末章合成 L3；`config.py` 新增 `ARC_SIZE`；env 模板两处；测试 +21（known_by 8 / 注入与写回 7 / 单章路径 6），全量 311 用例通过 |
| **可逆性** | 高。全部为新增注入段与参数化，无数据迁移；移除注入段即回退现状 |
